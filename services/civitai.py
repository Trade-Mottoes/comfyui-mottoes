...
def get_civitai_info(path: Path | str | None, model_hash: str) -> dict[str, Any] | None:
    try:
        if not model_hash:
            print("ComfyUI-Mottoes: Error: Missing hash.")
            return None
        ...
        if path is None:
            ...
            if manual_data is None:
                ...
                if file is None:
                    print(f"ComfyUI-Mottoes: ({model_hash}) No file hash matched in metadata (should be impossible)")
                    return content
                filename = file["name"]
        ...
        print(f"ComfyUI-Mottoes: Civitai info error: {e}")
    return None
...
def download_model_info(path: Path | str | None, model_hash: str) -> dict[str, object] | None:
    ...
    print(f"ComfyUI-Mottoes: Downloading model info for '{model_label}'.")
...
def save_civitai_info_file(content: dict[str, object], path: Path | str) -> bool:
    try:
        ...
        print(f"ComfyUI-Mottoes: Save Civitai info error '{path}': {e}")
        return False
    return True
...
def get_manual_list() -> dict[str, dict[str, Any]]:
    ...
    try:
        ...
        print(f"ComfyUI-Mottoes: Manual list get error: {e}")
    return {}
...
def append_manual_list(key: str, value: dict[str, Any]) -> dict[str, dict[str, Any]]:
    ...
    try:
        ...
        print(f"ComfyUI-Mottoes: Manual list append error: {e}")
    return manual_list
